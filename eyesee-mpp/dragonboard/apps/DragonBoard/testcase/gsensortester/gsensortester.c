...
#ifdef DEBUG_SENSOR
            printf("sensorName:%s,classPath:%s,lsg:%f\n",
...
#ifdef DEBUG_SENSOR
    printf("buf:%s, flag0:%d, flag1:%d\n", buf, flag0, flag1);
    printf("otherDevice[0].isFind:%d,otherDevice[1].isFind:%d\n",
...
#ifdef DEBUG_SENSOR
                        printf("lsg: %f,convert:%f", gsensorInfo.priData, convert);
#endif
                close(data_fd);
                data_fd = -1;
                        break;
...
#ifdef DEBUG_SENSOR
    printf("readEvents:  x,y,z:  %f, %f, %f, time : %d\n", mPendingEvent.acceleration.x,
...
#ifdef DEBUG_SENSOR
    printf("Sensor data:  x,y,z:  %f, %f, %f\n", mPendingEvent.acceleration.x,
                            mPendingEvent.acceleration.y,
...
{
    printf("writeGsensorIntEnable %d",isEnable);
    m_gsensorIntEnable = isEnable;

    if(mGsensorParkingImpactSlope==G_SENSOR_SENSI_PARKING_OFF_VAL)
    {
        printf("by hero  GsensorIntEnable is close, do not set intEnable \n");
        isEnable=0;
    }
    ...
    printf("by hero writeGsensorIntEnable isEnable=%d \n", isEnable);
...
int writeParkingSensibility(int value)
{
    ...
    if(gsensorInfo.classPath[0] == ICHAR)
        printf("by hero writeSensibility classpath err\n");
    ...
    switch(value)
    {
        ...
        case 0:
            ...
            break;
        default:
            printf("Invalid gsensor writeParkingSensibility:%d", value);
            return -1;
    }

    if(mGsensorParkingImpactSlope!=G_SENSOR_SENSI_PARKING_OFF_VAL)
    {
        printf("writeParkingSensibility set mGsensorParkingImpactSlope\n");
        ...
        printf("writeGsensorIntEnable false");
        writeGsensorIntEnable(0);
    }


    printf("by hero writeParkingSensibility value=%d mGsensorParkingImpactSlope=%d \n", value, mGsensorParkingImpactSlope);
...
int readParkingSensibility(void)
{
    ...
    printf("by hero readParkingSensibility value=%d \n", value);
...
{
    if(gsensorInfo.classPath[0] == ICHAR)
        printf("by hero writeDelay classpath err\n");
        //return -1;
...
int set_sysfs_input_attr(char *class_path, char *attr, char *value, int len)
{
    char path[256];
    int fd;
    printf("by hero set_sysfs_input_attr class_path = %s", class_path);
    ...
    fd = open(path, O_RDWR);
    printf("by hero set_sysfs_input_attr path = %s", path);
    if (fd < 0) {
...
int get_sysfs_input_attr(char *class_path, char *attr, int *ivalue)
{
    ...
    if (read(fd, value, len) < 0) {
            printf("path:%s", path);
            printf("Could not read SysFs attribute \"%s\" (%s).", attr, strerror(errno));
        close(fd);
        return -errno;
    }
...
int writeImpactHappenLevel(int level)
{
    ...
    printf("GsensorManager level %d",level);
    switch(level)
    {
        case 3:
        ...
        case 0:
            ...
            break;
        default:
            printf("Invalid gsensor impact_happen_level:%d", level);
            return -1;
    }
    printf("by owl writeImpactHappenLevel impact_happen_level mGsensorImpactTarget=%d \n", mGsensorImpactTarget);
    return 0;
}
...
int readImpactHappenLevel(void)
{
    ...
    if(gsensorInfo.classPath[0] == ICHAR)
        printf("by owl readImpactHappenLevel classpath err\n");

    get_sysfs_input_attr(gsensorInfo.classPath,"impact_happen_level", &value);

    printf("by owl readImpactHappenLevel value=%d \n", value);
    return value;
}
...
void *GsensorManagerThread(void *context)
{
    ...
    printf("by hero GsensorManagerThread path = %s", path);
    while(1)
    {
        //sleep(1);
        usleep(100*1000);
...
//                Mutex::Autolock _l(mLock);
                ...
                printf("GsensorManagerThread read Int impact status=1 !!!!!!! \n");
                write(fd, "0", 1);
            }
            close(fd);
            fd = -1;
...
void *NormalModeImpactEventThread(void *context)
{
    ...
    printf("by hero NormalModeImpactEventThread gsensor_fd node = %s", gsensorInfo.devname);
    int gsensor_fd = open(gsensorInfo.devname, O_RDONLY);
    if (gsensor_fd < 0)
    {
        printf("open gsensor input error!!!!!!! \n");
        return NULL;
    }

    printf("by hero NormalModeImpactEventThread path = %s", path);
    while(1)
    {
        sleep(1);
...
//                Mutex::Autolock _l(mLock);
                m_bImpactStatus = true;
                printf("NormalModeImpactEventThread read impact status=1 !!!!!!! \n");
                write(fd, "0", 1);
            }
            close(fd);
            fd = -1;
...
void *DealNormalAxisDataThread(void *context)
{
    ...
    printf("[OWL_DEBUG] 00 DealNormalAxisDataThread axis_x_key = %s", axis_x_key);
    int count = 0,test = 0;
...
*/
        ...
        {
            //printf("[OWL_DEBUG] gsensor axis fd NULL ");
            continue;
        }
        else
        {
            ...
            printf("========new:gx %f gy %f gz %f,old:gx %f gy %f gz %f========",
...
//          db_info("[OWL_DEBUG] DealNormalAxisDataThread gx=%f gy=%f gz=%f ", gx, gy, gz);
            ...
            if(count >= 5)
            {
                ...
                printf("gsensor test:gsensor fail");
                goto exit;
            } else if(test >= 5){
                fwrite(str_pass,1,strlen(str_pass),fd);
                printf("gsensor test:gsensor pass");
                goto exit;
            }
            close(x_fd);
            close(y_fd);
...
void Check_event(float x, float y, float z)
{
    ...
    else
    {
        ...
        //printf("dtmax = %.2f  mGsensorImpactTarget=%f axis=<%c>", dtmax, mGsensorImpactTarget, mGsensorOccurAxis);
        if (dtmax > mGsensorImpactTarget)
        {
//            Mutex::Autolock _l(mLock);
            m_bImpactStatus = true;
            printf("G-sensor Normal impact dtmax=%f mGsensorImpactTarget=<%f> gc=%d axis=<%c> impact_status=1  !!!!!!  \n\n",dtmax, mGsensorImpactTarget, gc, mGsensorOccurAxis);
            gc = 0;
            return;
        }
