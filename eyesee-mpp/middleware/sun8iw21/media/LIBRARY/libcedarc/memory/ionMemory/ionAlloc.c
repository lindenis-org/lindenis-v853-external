...
static void*                g_pVeopsSelf = NULL;
static VeOpsS*              g_veOps = NULL;
                                          ...
                                          void *veOps, void *pVeopsSelf, unsigned long *pAddr)
{
    if(CdcIonGetMemType()==MEMORY_IOMMU)
    {
        ...
        if (pVeopsSelf == NULL) {
            pVeopsSelf = g_pVeopsSelf;
        }

        if (veOps != NULL && pVeopsSelf != NULL)
        {
...
void ion_alloc_close()
{
    ...
    if (--g_alloc_context->ref_cnt <= 0)
    {
        ...
        CdcVeRelease(g_veOps, g_pVeopsSelf);
        g_veOps = NULL;
        g_pVeopsSelf = NULL;
...
static int ion_alloc_get_fd(void* pbuf)
{
    ...
    aw_mem_list_for_each_entry(tmp, &g_alloc_context->list, i_list)
    {
        if (addr_vir >= tmp->vir
            ...
            buffer_fd = tmp->iommu_buffer.fd;//tmp->dma_buf_fd;
