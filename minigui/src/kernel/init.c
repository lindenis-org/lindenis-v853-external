...
int button1_count = 0;//Êó±êµã»÷(´¥ÃþÆÁ°´ÏÂ)µÄ´ÎÊý
int button0_count = 0;//Êó±êËÉ¿ª(´¥ÃþÆÁËÉ¿ª)µÄ´ÎÊý
#endif
int move_count = 0;
/******************************* extern data *********************************/
...
//                }
                ...
                if(h_move_count >= v_move_count) { // Ë®Æ½·½ÏòµÄ´ÎÊý¶à,¡¡ÔòÈÏÎªÊÇË®Æ½»¬¶¯
                    v_move_count = 0;
                    if(deltX>0)
                ...
                } else { //scroll in the vertical //¡¡´¹Ö±·½ÏòµÄ´ÎÊý¶à,¡¡ÔòÈÏÎªÊÇ´¹Ö±»¬¶¯
                    h_move_count = 0;
                    if(deltY>0)
                        direction = MOUSE_DOWN;
                    else
        ...
        if(Msg.message == MSG_LBUTTONUP) {
#ifdef FORBID_TWOCLICKS_FLING
            //Êó±êµã»÷(´¥ÃþÆÁ°´ÏÂ)Ê±Ö»ÒªÃ»ÓÐËÉ¿ªÊÕµ½µÄÏûÏ¢¶¼ÊÇME_LEFTDOWN£¬µ±ÕæÕýflingÊ±£¬»áÊÕµ½¶à´ÎME_LEFTDOWN£¬Ö»ÓÐËÉ¿ªÊ±²Å»áÊÕµ½Ò»´ÎME_LEFTUP£¬
            //±ÜÃâÁ½Ö»ÊÖÍ¬Ê±µã»÷ÆÁÄ»Ê±£¬ÓÉÓÚME_LEFTDOWNºÍME_LEFTUP¼äµÄ×ø±ê²î²úÉúÁËMSG_MOUSE_FLINGÏûÏ¢£¬²ÉÓÃÒÔÏÂ²ßÂÔ¡£(>= 5¿ÉÒÔ¸ù¾ÝÊµ¼ÊÇé¿ö¾ßÌåµ÷½Ú)
            if ((button1_count - button0_count) >= 5) {
                ...
                if(abs(deltX)>20 || abs(deltY)>20) {
                    ...
                    Msg.wParam  = MAKELONG (direction, velocity);
                    Msg.lParam  = MAKELONG (me->x, me->y);
                    if(move_count <= 2){//ä¿®å¤éƒ¨åˆ†å±ä½¿ç”¨ä¹…äº†å‡ºçŽ°å°†ç‚¹å‡»è¯¯æŠ¥ä¸ºæ»‘å±äº‹ä»¶,è¿™é‡Œé€šè¿‡è®¡æ•°moveæ¬¡æ•°è¿›è¡Œè¿‡æ»¤
                        printf("\n\n wlw ignor abnormal MSG_MOUSE_FLING1 (move_count:%d)\n\n",move_count);
                    }else{
                        QueueDeskMessage (&Msg);
                    }
                    move_count = 0;
