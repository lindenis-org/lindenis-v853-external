...
static void ion_alloc_close()
{
    ...
    aw_mem_list_for_each_safe(pos, q, &g_context->list)
    {
        ...
        logv("ion_alloc_close del item phy= 0x%lx vir= 0x%p, size= %d \n",
...
static void ion_alloc_pfree(void * pbuf, void *veOps, void *pVeopsSelf)
{
    ...
    aw_mem_list_for_each_entry(tmp, &g_context->list, i_list)
    {
        if (tmp->vir == addr_vir)
        {
            logv("free phy= 0x%lx vir= 0x%p, size= %d \n",tmp->phy, tmp->vir, tmp->size);
            /*unmap user space*/
#if KERNEL_VERSION_OLD
            if(CdcIonGetMemType() == MEMORY_IOMMU )
            {
                logv("ion_pfree: fd:%d, iommu_addr:%lx\n", tmp->dma_buf_fd, tmp->phy);
...
static int ion_alloc_get_vir(int dmafd, void* viraddr)
{
    return 0;
}
...
static int ion_alloc_get_fd(void* pbuf)
{
    ...
    aw_mem_list_for_each_entry(tmp, &g_context->list, i_list)
    {
        if (addr_vir >= tmp->vir
            ...
            logv("ion_alloc_get_fd, fd = 0x%08x vir= 0x%p\n", buffer_fd, addr_vir);
            flag = 1;
            break;
        }
    }
