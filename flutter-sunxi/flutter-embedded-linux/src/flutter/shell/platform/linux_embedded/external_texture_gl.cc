...
static const GlProcs& GlProcs() {
  ...
  if (!initialized) {
    ...
    if (!procs.glGenTextures)
      procs.glGenTextures =
          reinterpret_cast<glGenTexturesProc>(::dlsym(RTLD_DEFAULT, "glGenTextures"));
    procs.glDeleteTextures = reinterpret_cast<glDeleteTexturesProc>(
        eglGetProcAddress("glDeleteTextures"));
    if (!procs.glDeleteTextures)
      procs.glDeleteTextures =
          reinterpret_cast<glDeleteTexturesProc>(::dlsym(RTLD_DEFAULT, "glDeleteTextures"));
    procs.glBindTexture =
        reinterpret_cast<glBindTextureProc>(eglGetProcAddress("glBindTexture"));
    if (!procs.glBindTexture)
      procs.glBindTexture =
          reinterpret_cast<glBindTextureProc>(::dlsym(RTLD_DEFAULT, "glBindTexture"));
    procs.glTexParameteri = reinterpret_cast<glTexParameteriProc>(
        eglGetProcAddress("glTexParameteri"));
    if (!procs.glTexParameteri)
      procs.glTexParameteri =
          reinterpret_cast<glTexParameteriProc>(::dlsym(RTLD_DEFAULT, "glTexParameteri"));
    procs.glTexImage2D =
        reinterpret_cast<glTexImage2DProc>(eglGetProcAddress("glTexImage2D"));
    if (!procs.glTexImage2D)
      procs.glTexImage2D =
          reinterpret_cast<glTexImage2DProc>(::dlsym(RTLD_DEFAULT, "glTexImage2D"));
