...
void sunxifb_init(uint32_t rotated) {
  ...
  if (fbfd == -1) {
    perror("flutter: cannot open framebuffer device");
    return;
  }

  // Get fixed screen information
  if (ioctl(fbfd, FBIOGET_FSCREENINFO, &finfo) == -1) {
    perror("flutter: reading fixed information");
    return;
  }

  // Get variable screen information
  if (ioctl(fbfd, FBIOGET_VSCREENINFO, &vinfo) == -1) {
    perror("flutter: reading variable information");
    return;
  }
  ...
  if (ioctl(fbfd, FBIO_ENABLE_CACHE, args) < 0) {
    perror("flutter: FBIO_ENABLE_CACHE fail");
  }
  ...
  fbp = (char*) mmap(0, screensize, PROT_READ | PROT_WRITE, MAP_SHARED, fbfd,
      0);
  if ((intptr_t) fbp == -1) {
    perror("flutter: failed to map framebuffer device to memory");
    return;
  }

  printf("flutter: wh=%dx%d, vwh=%dx%d, bpp=%d, rotated=%d\n", vinfo.xres,
      vinfo.yres, vinfo.xres_virtual, vinfo.yres_virtual, vinfo.bits_per_pixel,
      rotated);
  ...
  if (sinfo.fbnum > 1) {
    printf("flutter: turn on double buffering.\n");
...
#ifdef USE_SUNXIFB_G2D
    printf("flutter: turn on 2d hardware acceleration.\n");

    if ((sinfo.g2dfd = open("/dev/g2d", O_RDWR)) < 0) {
      perror("flutter: cannot open g2d device");
    }
...
#ifdef USE_SUNXIFB_G2D_ROTATE
    printf("flutter: turn on 2d hardware rotation acceleration.\n");

    sinfo.memops = GetMemAdapterOpsS();
    if (SunxiMemOpen(sinfo.memops) < 0) {
      perror("flutter: cannot open ion device");
    }

    sinfo.rotatefbp = SunxiMemPalloc(sinfo.memops,
        finfo.line_length * vinfo.yres);
    if (sinfo.rotatefbp == 0) {
      perror("flutter: cannot malloc rotate buffer");
      return;
    }

    fbp = sinfo.rotatefbp;
    sinfo.rotatefbp_phy = (uintptr_t) SunxiMemGetPhysicAddressCpu(sinfo.memops,
        sinfo.rotatefbp);
...
/**
 * Flush a buffer to the allocation
 * @param user_data user data
 * @param allocation image data pointer
 * @param row_bytes number of bytes in a row
 * @param height generally the height of the screen
 */
bool sunxifb_present(void *user_data, const void *allocation, size_t row_bytes,
    size_t height) {
  if (fbp == NULL || allocation == NULL)
    return false;

  memcpy(fbp, allocation, row_bytes * height);

  //May be some direct update command is required
  //ret = ioctl(state->fd, FBIO_UPDATE, (unsigned long)((uintptr_t)rect));

#ifdef USE_SUNXIFB_DOUBLE_BUFFER
  if (sinfo.fbnum > 1 && sinfo.dbuf_en) {
#ifdef USE_SUNXIFB_CACHE
    uintptr_t args[2];
    args[0] = (uintptr_t) sinfo.screenfbp[sinfo.fbindex];
    args[1] = finfo.line_length * vinfo.yres;
    if (ioctl(fbfd, FBIO_CACHE_SYNC, args) < 0) {
      perror("flutter: FBIO_CACHE_SYNC fail");
    }
#endif /* USE_SUNXIFB_CACHE */
    ...
    if (ret == 0) {
      if ((outfp = fopen("/mnt/UDISK/out", "wb+")) == NULL) {
        printf("flutter: open file fail. \n");
      } else {
        printf("flutter: open file ok. \n");
      }
      printf("flutter: ==out_size=%d, addr=%p==\n", finfo.line_length * vinfo.yres,
          sinfo.rotatefbp);
      ret = fwrite(sinfo.rotatefbp, finfo.line_length * vinfo.yres, 1, outfp);
      printf("flutter: fwrite,ret=%d\n", ret);
      fclose(outfp);
    }
#endif
    ...
    if (ioctl(fbfd, FBIOPAN_DISPLAY, &vinfo) < 0) {
      perror("flutter: FBIOPAN_DISPLAY fail");
    }
...
int sunxifb_set_dbuf_en(bool dbuf_en) {
  ...
  fbp = (char*) mmap(0, finfo.smem_len, PROT_READ | PROT_WRITE, MAP_SHARED, fbfd,
      0);
  if ((intptr_t) fbp == -1) {
    perror("flutter: failed to map framebuffer device to memory");
    return -1;
...
static int sunxifb_g2d_blit(int index) {
  ...
  if (ioctl(sinfo.g2dfd, G2D_CMD_BITBLT_H, (uintptr_t)(&info)) < 0) {
    perror("flutter: G2D_CMD_BITBLT_H failed");
    printf(
        "flutter: src_addr=%u src.format=%d src_wh=[%d %d] src_clip=[%d %d %d %d] "
