...
enum video_format_e {
	_videoFmtRGB, _videoFmtYUV444, _videoFmtYUV422, _videoFmtYUV420,
};
...
enum sink_type_e {
	SINK_TYPE_DVI = 1, SINK_TYPE_HDMI = 2,
};
...
public:
	struct videoFormat {
		...
		videoFormat() :
				format(0), dc_30bit(0), dc_36bit(0), dc_48bit(0), name(0) {
		}
	};
	...
	struct videoInformationCode {
		...
		videoInformationCode(int v, int f, int r, int y, const char *n) :
				vic(v), native(f), regular_sampling(r), ycbcr420_sampling(y), name(
						n) {
		}
	};
	...
	void ceaSvd(const unsigned char *x, int n, int r, int y,
