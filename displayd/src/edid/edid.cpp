...
static int do_checksum(unsigned char *x) {
	...
	for (i = 0; i < 128; i++)
		sum += x[i];
	if (sum) {
		dd_error("Checksum = %02x (should be 0x%02x)", x[0x7f],
...
EdidParser::EdidParser(const char *path) :
		mPath(path), mStandardTiming(), mDetailedTiming(), mHdmi4k2kVIC(), mSupportedVIC(), mY420VIC(), mSupportedFormat() {
	clear();
}
...
void EdidParser::clear() {
	...
	for (std::vector<videoInformationCode *>::iterator it =
			mHdmi4k2kVIC.begin(); it != mHdmi4k2kVIC.end();) {
	...
	for (std::vector<videoInformationCode *>::iterator it =
			mSupportedVIC.begin(); it != mSupportedVIC.end();) {
...
int EdidParser::parseMonitorInfo() {
	...
	/* Serial number */
	mSerialNumber = (edid[15] << 24) | (edid[14] << 16) | (edid[13] << 0)
			| edid[12];
...
int EdidParser::parseStandardTiming() {
	...
	const struct standard_timing _standard_timing_tab[] = {
			...
			{ HDMI_4k2k_25, 0x03, 3840, 25 },
			{ HDMI_4k2k_30, 0x03, 3840, 30 },
	};
	...
	for (size_t i = 0; i < length; i++) {
		...
		for (size_t j = 0; j < NEMLEN(_standard_timing_tab); j++) {
			...
			if ((info->horizontal_pixel == width) && (info->frame_rate == freq)
					&& (info->ratio == ratio)) {
...
int EdidParser::parseDescriptorBlock(size_t begin) {
	...
	char *str = 0;
	int ret = asprintf(&str, "%4dx%4d pixel clock %.3f MHz %s", hsync_active,
			vsync_active, pixel_clock, interlaced ? "interlaced" : "");
...
EdidParser::videoFormat *EdidParser::getVideoFormat(int format) {
	...
	case _videoFmtRGB:
		newfmt->name = "RGB        ";
		break;
	case _videoFmtYUV444:
		newfmt->name = "YCbCr 4:4:4";
		break;
	case _videoFmtYUV422:
		newfmt->name = "YCbCr 4:2:2";
		break;
	case _videoFmtYUV420:
		newfmt->name = "YCbCr 4:2:0";
		break;
	default:
		newfmt->name = "unknow     ";
		break;
...
void EdidParser::hdmi4k2kVICParse(const unsigned char *x, int size) {
	const int __extended_vic[] = { 0, HDMI_3840x2160p30_16x9,
			HDMI_3840x2160p25_16x9, HDMI_3840x2160p24_16x9,
...
void EdidParser::getVideoFormatSupported() {
	...
	for (std::vector<videoInformationCode *>::iterator it =
			mSupportedVIC.begin(); it != mSupportedVIC.end(); ++it) {
		videoInformationCode *v = *it;
		if (v->native)
...
void EdidParser::dump() {
	...
	for (std::vector<int>::iterator it = mStandardTiming.begin();
			it != mStandardTiming.end(); ++it)
		printf("    %s\n", getCeaVicName(*it));

	printf("\n");
	printf("  Detailed Timing Descriptor:\n");
	for (std::vector<char *>::iterator it = mDetailedTiming.begin();
			it != mDetailedTiming.end(); ++it)
		printf("    %s\n", *it);
	...
	for (int i = 0; i < 8; i++) {
		const char* __colorimetry[] = { "XVYCC601", "XVYCC709", "SYCC601",
				"ADOBE_YCC601", "ADOBE_RGB", "BT2020_CYCC", "BT2020_YCC",
				"BT2020_RGB", };
		if (mColorimetry & (1 << i))
			printf("    %s\n", __colorimetry[i]);
	}
	...
	for (int i = 0; i < 4; i++) {
		const char* __eotf[] = { "SDR_LUMINANCE_RANGE", "HDR_LUMINANCE_RANGE",
				"SMPTE_ST_2084", "FUTURE_EOTF" };
		if (mEotf & (1 << i))
			printf(" %s", __eotf[i]);
	}
	printf("\n");
