...
hotplugListener::hotplugListener(int listenerSocket) :
		NetlinkListener(listenerSocket, NetlinkListener::NETLINK_FORMAT_ASCII), mHotplugCallback(
				0), mSocket(listenerSocket), mFrameworkListener(0) {
}
...
void hotplugListener::registerHotplugCallback(void *pfn) {
	mHotplugCallback = reinterpret_cast<HOTPLUG_CHANGE_CALLBACK>(pfn);
	return;
}

void hotplugListener::registerFrameworkListener(SocketListener *l) {
	mFrameworkListener = l;
	return;
}

void hotplugListener::sendBroadcast(const char *msg) {
	if (mFrameworkListener)
		mFrameworkListener->sendBroadcast(618, msg, false);
	return;
}

void hotplugListener::broadcastHotplugMsg(const char *name, const char *state) {
...
void hotplugListener::onEvent(NetlinkEvent *event) {
	...
	if(!strcmp(subsys, "switch")) {
		if(event->getAction() == NetlinkEvent::Action::kChange) {
			...
			if((NULL == name) || (NULL == state)) {
				ALOGE("null point: name=%s, state=%s.\n", name, state);
				return;
			}
			ALOGD("hotplug change: name=%s, state=%s.\n", name, state);
	...
	if(!strcmp(subsys, "extcon")) {
		if(event->getAction() == NetlinkEvent::NlActionChange) {
...
void hotplugListener::notifyHotplugChanged(const char *name, bool plugin) {
	ALOGD("hotplugListener: %s %s\n", name, plugin ? "connect" : "disconnect");
	if (mHotplugCallback)
		mHotplugCallback(name, plugin);
	return;
}

int hotplugListener::createUeventSocket(void) {
