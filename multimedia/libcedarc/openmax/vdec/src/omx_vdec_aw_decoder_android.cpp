...
//* replace include by below commented ones for A50 sunxi-platform-q compile
...
static void showBufferInfoArrStatus(OmxAwDecoderContext *pCtx)
{
    ...
    for(i = 0; i < OMX_MAX_VIDEO_BUFFER_NUM; i++)
    {
        ...
        if(pCtx->mOutputBufferInfoArr[i].mStatus == OWNED_BY_US)
            ...
            nOwnedByPending++;
        logv("i:%2d, pHeadBufInfo:%10p, pic:%10p, status: %s fd: %d",
            i,
    ...
    logd("buffers owned by render [%d] by decoder [%d] by us[%d] by tnr[%d] by pending[%d].",
        ...
        if(mRet == 0)
        {
            ...
            else // start case:
            {
                pVideoPicture = SetVideoFbmBufAddress(pCtx->m_decoder,
                                                      &mVideoPicture,
                                                      nForbiddenUseFlag);
                logd("*** call SetVideoFbmBufAddress: pVideoPicture(%p), bufFd: %d",
...
static int anCheckResolutionChange(OmxAwDecoderContext *pCtx, VideoPicture* picture)
{
    ...
    else
    {
           ...
           (OMX_U32)nPicRealHeight != outDef->format.video.nFrameHeight)
        {
            ...
            {
                logd("not bUseAndroidBuffer: %lu bit aligned width and height"
                                                    ...
                                                    OMX_BUFFERHEADERTYPE* pOutBufHdr)
{
    ...
    {
        logw("Lock GUIBuf fail. pBufferHandle [%p]!", pBufferHandle);
    }
    if(!dst)
    {
        loge("the buffer dst is NULL. we would NOT copy picture to buffer.");
    }
    else
    {
...
static OMX_BUFFERHEADERTYPE* drainOutBufferInterlace(OmxAwDecoderContext *pCtx)
{
    ...
    if(pCtx->pOutBufHdr4DiOutPic1)
    {
        ...
        logv("return out buffer stored. addr %p. pts %lld.",
    ...
    if(i>=DI_BUFFER_NUM_KEEP_IN_OMX)
    {
        logv("cannot found the free di buffer. FreeDiCount %lu.", nFreeDiCount);
        return NULL;
    }

    logv("nFreeDiCount [%lu]. pDiOutPicture [%p]. pDiOutPicture1 [%p]. "
    ...
    if(bOnly1OutputPic)
        logv("**after deinterlace: cdc di processes 3 input pictures, %d, %d, %d. output pics %d",
            pCtx->pPrePrePicture->nBufFd, pCtx->pPrePicture->nBufFd, pCtx->pCurPicture->nBufFd,
            pDiOutPicture->nBufFd/* , pDiOutPicture1->nBufFd, pDiOutTnrPicture->nBufFd*/);
    else
        logv("**after deinterlace: cdc di processes 3 input pictures, %d, %d, %d. output pics %d %d %d.",
            pCtx->pPrePrePicture->nBufFd, pCtx->pPrePicture->nBufFd, pCtx->pCurPicture->nBufFd,
            pDiOutPicture->nBufFd , pDiOutPicture1->nBufFd, pDiOutTnrPicture->nBufFd);
    ...
    // 4. return picture
    // In DI300, return or cancel pre-pre-pic. set pre pic as pre-pre-pic, set ditnrpic as pre-pic.
    if(pCtx->pPrePrePicture != pCtx->pPrePicture && pCtx->pPrePrePicture != NULL)
    {
        ...
        if(pCtx->mDi300ProcessCount >= 2)
        {
            ...
            logv("return picture [%d] of 3rd di input to decoder.", pCtx->pCurPicture->nBufFd);
        }
    }
    ...
    {
        logv("pre-pre-picture=%p, pre-pic=%p, cur-pic=%p",
    ...
    {
        logv("pre-pre-picture=%p, pre-pic=%p, cur-pic=%p, DiOutPic=%p.",
    ...
    if(!bOnly1OutputPic)
    {
        ...
        logv("di out picture 1 pts %lld  picture 2 pts %lld.",
    ...
    if(pOutBufHdr->pOutputPortPrivate != NULL)
    {
        ...
        logv("***hdr info for di output pic 0: %lu, %lu, %lu",
            pHdrInfo->nExtMatrixCoeffs,
            pHdrInfo->nExtVideoFullRangeFlag,
            pHdrInfo->nExtTransferCharacteristics);
    }
    if(!bOnly1OutputPic && pOutBufHdr1->pOutputPortPrivate != NULL)
    {
        ...
        logv("***hdr info for di ouptut pic 1: %lu, %lu, %lu",
    ...
    logv("return out buffer. addr %p. pts %lld.  %lu.", pOutBufHdr, pOutBufHdr->nTimeStamp/1000, pOutBufHdr->nFilledLen);
    return pOutBufHdr;
}
...
static void standbyOutBufferArr(OmxAwDecoderContext *pCtx)
{
    ...
    for(i = 0; i < OMX_MAX_VIDEO_BUFFER_NUM; i++)
    {
        ...
        {
            logd("** fillbufferDone owned_by_us or tnr, i[%d]", i);
            pCtx->callback(pCtx->pUserData, AW_OMX_CB_FILL_BUFFER_DONE,
                            pCtx->mOutputBufferInfoArr[i].pHeadBufInfo);
...
static int __anPrepare(OmxDecoder* pDec)
{
    ...
    if(pCtx->bUseZeroCopyBuffer)
    {
        ...
        logd("****mExtraOutBufferNum:%lu", pCtx->mExtraOutBufferNum);
    }
...
static void __anUpdateFormat(OmxDecoder* pDec)
{
    ...
    if(pCtx->bUseAndroidBuffer)
    {
        ...
        logv("*****acutalcolor:%lx, update:%lx", pCtx->nActualColorFormat, color);
