...
#define RETURN_NULL_FOR_NULL_PARSER \
    do {if(mPriData->mNullParser == true) return NULL;}while(0)
...
typedef struct MetadataPriData{
        ...
        CdxStreamT*                 mStream;
#endif
        bool                        mNullParser;//If yes, all info method would return NULL.
}MetadataPriData;
...
AwMetadataRetriever::~AwMetadataRetriever()
{
    ...
    logd("AwMetadataRetriever destroyed.\n\n");
}
...
#endif
{
    ...
    {
        mPriData->mNullParser = true;
        loge("parser for the source cannot be created. source can be set yet.");
        return OK;
    }
...
static char *FdToFilepath(const int fd)
{
    ...
    if (ret == -1)
    {
        logw("readlink failure, errno(%d)", errno);
        return NULL;
    }
...
void AwMetadataRetriever::storeMetadata(void)
{
    ...
    //if parse the title name fail, get the title from the file name.
    char *FilePath;
    if(hasTitle == 0)
    {
        char *ptr;
    ...
    logv("detect conventor size : %d", size);
    if (size) {
...
const char* AwMetadataRetriever::extractMetadata(int keyCode)
{
    ...
    if (index < 0)
    {
        ...
        {
            logv("no metadata for %s.", strMetadataName[keyCode]);
        }
        return NULL;
    }
...
sp<IMemory> AwMetadataRetriever::getStreamAtTime(int64_t timeUs)
{
    ...
    if(pOutBuf && nBitStreamLength)
    {
        ...
        else
        {
            ...
            pData = static_cast<unsigned char*>(mem->unsecurePointer());
#elif
...
VideoFrame* AwMetadataRetriever::getFrameAtTime(int64_t timeUs, int option)
{
    ...
    logv("getFrameAtTime, mFd=%d", mPriData->mFd);
...
VideoFrame* AwMetadataRetriever::getFrameAtTime(int64_t timeUs, int option, int colorFormat, bool metaOnly)
{
    ...
    if(metaOnly == true)
    {
        logv("==== metaOnly is true, cannot support");
        return NULL;
    }
            ...
            int index, int colorFormat, bool metaOnly, bool thumbnail)
{
    ...
    CDX_UNUSE(metaOnly);
    logv("++++ unsupport function getImageAtIndex");
    return NULL;
}
            ...
            int index, int colorFormat, int left, int top, int right, int bottom)
{
    ...
    CDX_UNUSE(right);
    CDX_UNUSE(bottom);
    logv("++++ unsupport function getImageRectAtIndex");
    return NULL;
}
#endif
#if (CONF_ANDROID_MAJOR_VER == 9 || CONF_ANDROID_MAJOR_VER == 10)
            ...
            int frameIndex, int numFrames, int colorFormat, bool metaOnly)
{
    ...
    CDX_UNUSE(numFrames);
    CDX_UNUSE(metaOnly);
    logv("++++ unsupport function getFrameAtIndex");
    return 0;
}
#endif

#if (CONF_ANDROID_MAJOR_VER >= 11)
sp<IMemory> AwMetadataRetriever::getFrameAtIndex(
