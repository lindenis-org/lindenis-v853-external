...
#if (0) //not use multi thread by default
...
static int handlePrepare(DownloadCtxT *d)
{
    ...
    do
    {
        ...
        logv("request+read response: %lld ms", (CdxGetNowUs()-t0)/1000);
...
static int CdxGetProbeData(CdxHttpStreamImplT *impl)
{
    ...
    if (!CdxStreamSeekAble(&impl->base) && (impl->baseOffset > 0)) {
        while(size < impl->baseOffset)
        {
            if(impl->forceStopFlag == 1)
            {
                logv("forceStop.");
            ...
            ret = CacheManagerSkipForProbeCache(impl->cacheManager, impl->baseOffset - size);
            if(ret < 0)
            {
                loge("skip baseOffset data fail.");
                goto err_out;
            }
            else if(ret == 0)
            {
            ...
            logv("skip data need size=%lld, size=%d, totSize=%lld",
                impl->baseOffset - size, ret, impl->totalSize);
            size += ret;
        }
    ...
    while(size < impl->probeData.len)
    {
        ...
        else if(ret == 0)
        {
            ...
            if(impl->threadNum == 1)
            {
                ...
                //process DL_STATE_ERROR: seekReconnect
                for(i = 0; i < impl->threadNum; i++) {
                    if (impl->dlCtx[i].dlState == DL_STATE_ERROR) {
                        logw("CdxGetProbeData detect DL_STATE_ERROR");
                        ret = seekReconnect(impl, impl->readPos);
                        if(ret < 0) {
                            logw("CdxGetProbeData detect DL_STATE_ERROR, but seekReconnect failed!");
                            goto err_out;
                        } else {
                            int tmpRet;
                            pthread_mutex_lock(&impl->lock);
                            ...
                            if(tmpRet == 0) {//reconnect success
                                logd("CdxGetProbeData reconnect success");
                                break;
                            } else {
                                usleep(50000);
                            }
                        }
                        break;//if (impl->dlCtx[i].dlState == DL_STATE_ERROR) {
                    }
                }//end for
    ...
    logv("probe data len %d", impl->probeData.len);
#if __SAVE_BITSTREAMS
...
static cdx_int32 __CdxHttpStreamSeek(CdxStreamT *stream, cdx_int64 offset, cdx_int32 whence)
{
    ...
    {
        ret = 0;
        logv("offset[%lld] == impl->readPos[%lld]", offset, impl->readPos);
        goto out;
    }
                                        ...
                                        void **pVal)
{
    ...
    {
        *pVal = impl->sourceUri;
        logv("impl->sourceUri=%s", impl->sourceUri);
        return 0;
    }
    ...
    else if(strcmp(key, "statusCode") == 0)
    {
        if(httpHdr)
        {
            logv("++++++ statusCode in http: %d", httpHdr->statusCode);
...
static int threadNum(CdxHttpStreamImplT *impl)
{
    ...
    if(strstr(impl->sourceUri, "://127.0.0.1"))
        ret = 1;
#else
    ret = 1;
    logv("ret = 1");
