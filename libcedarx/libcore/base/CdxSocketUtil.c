...
#define TCP_INITIATIVE_SET_LOOPTIME (1) /* initiative set loop time*/
#define TCP_SELECT_LOOPTIME 2000
                        ...
                        cdx_long timeoutUs, cdx_int32 *pForceStop)
{
    ...
    fd_set ws;
    struct timeval tv;
    cdx_long loopTimes = 0, i = 0, j = 0;

#if TCP_INITIATIVE_SET_LOOPTIME
    loopTimes = TCP_SELECT_LOOPTIME;
#else
    if (timeoutUs == 0)
    {
    ...
    //time out
    CDX_LOGE("<%s,%d> timeout", __FUNCTION__, __LINE__);
    return -1;
}
