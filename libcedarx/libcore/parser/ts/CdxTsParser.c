...
CDX_INTERFACE void DestroyStream(Stream *mStream)
{
    ...
    if(mStream->mProgram->mTSParser->pDecryptor)
    {
        DestroyDecryptor(&mStream->mProgram->mTSParser->pDecryptor);
    }
    ...
    //类似于new_pes_av_stream
{
    ...
    case CDX_STREAM_TYPE_H264_ENCRYPTED:
        codec_type = TYPE_VIDEO;
    ...
    case CDX_STREAM_TYPE_AAC_ENCRYPTED:
        codec_type = TYPE_AUDIO;
    ...
    case CDX_STREAM_TYPE_AC3_ENCRYPTED:
        codec_type = TYPE_AUDIO;
...
/**
 * @brief getNextNALUnit: get the h264 nal
 *
 * @author ganqiuye(ganqiuye@allwinnertech.com)
 *
 * @date   2021.03.04
 *
 * @param data:      the origin data, can not change
 * @param size:      the size of origin data
 * @param nalStart:  the nal start ptr
 * @param nalSize:   the size of nalu
 *
 * @return: -1: can not find the start code
 *           0: ok
 */
static int getNextNALUnit(
        const uint8_t *data, size_t size,
    ...
    *nalSize = 0;
    ...
    const uint8_t* pFirstStartCode = findStartCodePrefix(data, size);
    if(pFirstStartCode == NULL)
    {
        *nalSize = size;
        return -1;
    }
    size_t offset = pFirstStartCode[2] == 0x1 ? 3: 4;
    size_t leftBytes = size - offset;
    size_t startOffset = offset;
    const uint8_t* pSecondStartCode = findStartCodePrefix(data + offset, leftBytes);
    if(pSecondStartCode == NULL)
    {
        *nalStart = &data[startOffset];
        *nalSize = leftBytes;
        return 0; //assume the nalu is end
    }
    *nalSize = pSecondStartCode - pFirstStartCode - startOffset;
    *nalStart = &data[startOffset];
...
/**
 * @brief dequeueAccessUnitH264: dequeue the h264 encrypt data
 *
 * @author ganqiuye(ganqiuye@allwinnertech.com)
 *
 * @date   2021.03.04
 *
 * @param mStream
 * @param data: the origin data, will be changed(decrypt)
 * @param size: the size of origin data
 *
 * @return: -1: error
 *           0: ok
 */
static int dequeueAccessUnitH264(Stream *mStream, uint8_t *data, size_t size)
{
    CdxDecryptorT* pDecryptor = mStream->mProgram->mTSParser->pDecryptor;
    ...
    while (1)
    {
        int ret = getNextNALUnit(data + offset, size - offset, &nalStart, &nalSize);
        if(ret == -1)
        {
            //here may has some bugs, fix it
            break;
        }
        else
        {
            ...
            {
                uint8_t *nalData = (uint8_t*)nalStart;//the content will be changed
                size_t newSize = pDecryptor->processNal(pDecryptor, nalData, nalSize);
                logv("newSize:%d", newSize);
            }
        }
        offset = nalStart - data + nalSize;
    }
    return 0;
}

static int findAACSyncWord(unsigned char *pBuf, int nBytes)
{
    int i;
    ...
    {
        if ( (pBuf[i+0] & 0xff) == 0xff && (pBuf[i+1] & 0xf0) == 0xf0 )
            return i;
    }
    return -1;
}
...
/**
 * @brief dequeueAccessUnitAAC: dequeue the AAC encrypt data
 *        for now, can not support the redundancy, fix it!
 *
 * @author ganqiuye(ganqiuye@allwinnertech.com)
 *
 * @date   2021.03.02
 *
 * @param mStream
 * @param data: the origine aac data, will be changed(decrypt)
 * @param size
 *
 * @return: -1:error
 *           0: ok
 */
static int dequeueAccessUnitAAC(Stream *mStream, uint8_t *data, size_t size)
{
    CdxDecryptorT* pDecryptor = mStream->mProgram->mTSParser->pDecryptor;
    size_t offset = 0;
    while (offset < size) {
        if (offset + 7 > size) {
            CDX_LOGD("maybe not return");
            //break;
            return -1;
        }

        uint8_t* bitstream = data + offset;
        size_t leftBytes = size - offset;
        int index = findAACSyncWord(bitstream, leftBytes);
        if(index == -1)
        {
            CDX_LOGW("can not find the sync work!!!");
            //break;
            return -1;
        }
        bitstream += index;
        leftBytes -= index;
        int protection_absent = bitstream[1]&0x01;
        unsigned aac_frame_length = ((int)(bitstream[3]&0x3)<<11)
                               |((int)(bitstream[4]&0xff)<<3)|((bitstream[5]>>5)&0x07);
        if(aac_frame_length == 0)
        {
            CDX_LOGE("aac_frame_length is 0!");
            //break;
            return -1;
        }
        //unsigned number_of_raw_data_blocks_in_frame
        if (offset + aac_frame_length > size) {
            CDX_LOGW("data is not enough!!");
            //break;
            return -1;
        }
        size_t headerSize = protection_absent ? 7 : 9;
        pDecryptor->processAAC(pDecryptor, headerSize, data + offset, aac_frame_length);
        offset += aac_frame_length;
    }
    if(offset != size)
    {
        //please fix it!!
        CDX_LOGI("offset = %d NOT equare size:%d", offset, size);
    }
    return 0;
}

/**
 * @brief dequeueAccessUnitEAC3: dequeue the EAC3 encrypt data,
 *        for now, can not support, fix it!
 *
 * @author ganqiuye(ganqiuye@allwinnertech.com)
 *
 * @date   2021.03.02
 *
 * @param mStream
 * @param data: the origine EAC3 data, will be changed(decrypt)
 * @param size
 *
 * @return: -1:error
 *           0: ok
 */
static int dequeueAccessUnitEAC3(Stream *mStream, const uint8_t *data, size_t size)
{
    CDX_UNUSE(mStream);
    CDX_UNUSE(data);
    CDX_UNUSE(size);
        ...
        const uint8_t *data, size_t size)
{
    ...
    if(mStream->bSampleEncrypted)
    {
        if(mStream->mProgram->mTSParser->pDecryptor == NULL)
        {
            mStream->mProgram->mTSParser->pDecryptor = CreateDecryptor(mStream->mProgram->mTSParser->mSampleKeyItem);
        }
    }
    if(mStream->mMediaType == TYPE_VIDEO
        && (mStream->codec_id == VIDEO_CODEC_FORMAT_H264
        ...
        || mStream->codec_id == VIDEO_CODEC_FORMAT_MPEG2))
    {
        ...
        if(mStream->pes[mStream->pesIndex].AUdata)
        {
            ...
            if(mStream->accessUnitStarted)
            {
                ...
                if(mStream->bSampleEncrypted && mStream->codec_id == VIDEO_CODEC_FORMAT_H264)
                {
                    dequeueAccessUnitH264(mStream, CdxBufferGetData(mStream->pes[!mStream->pesIndex].mBuffer),
                                          CdxBufferGetSize(mStream->pes[!mStream->pesIndex].mBuffer));
                }
                mStream->accessUnit.pts = mStream->pes[!mStream->pesIndex].pts;
                mStream->accessUnit.data =
    ...
    else
    {
        if(mStream->bSampleEncrypted)
        {
            if(mStream->codec_id == AUDIO_CODEC_FORMAT_MPEG_AAC_LC)
            {
                dequeueAccessUnitAAC(mStream, (uint8_t*)data, size);
            }
            else if(mStream->codec_id == AUDIO_CODEC_FORMAT_AC3)
            {
                dequeueAccessUnitEAC3(mStream, data, size);
            }
        }
...
/*
        ...
        while (nInfoBytesRemaining > 0)
        {
            ...
            stream->bSampleEncrypted =
            stream_type == CDX_STREAM_TYPE_H264_ENCRYPTED ||
            stream_type == CDX_STREAM_TYPE_AAC_ENCRYPTED  ||
            stream_type == CDX_STREAM_TYPE_AC3_ENCRYPTED;
...
cdx_int32 TSControl(CdxParserT *parser, cdx_int32 cmd, void *param)
{
    ...
    switch(cmd)
    {
        ...
        case CDX_PSR_CMD_SET_SAMPLE_ENCRYPT_INFO:
        {
            SampleEncryptInfoT item = *(SampleEncryptInfoT *)param;
            mTSParser->mSampleKeyItem = item;
            if(mTSParser->pDecryptor != NULL)
            {
                mTSParser->pDecryptor->signalNewSampleKey(mTSParser->pDecryptor, mTSParser->mSampleKeyItem);
            }
            break;
        }
        default:
