...
cdx_int32 EvaluateStrhValid(AVIStreamHeaderT *pabsHdr)
{
    ...
    {
        CDX_LOGD("pabsHdr->dwRate=%u, pabsHdr->dwScale=%u", pabsHdr->dwRate, pabsHdr->dwScale);
    ...
    if(pabsHdr->dwSampleSize > 0 || pabsHdr->dwScale < STRH_DWSCALE_THRESH)
    {
        ...
        {
            CDX_LOGD("pabsHdr->dwSampleSize=%u, pabsHdr->dwScale=%u, %u",
...
cdx_int32 GetIdx1Info(AviFileInT *aviIn)
{
    ...
    do
    {
        ...
        if(CdxStreamSeek(aviIn->fp, aviIn->dataChunk.length - 1, STREAM_SEEK_CUR))
        {
            aviIn->idx1Total = 0;
            CDX_LOGE("seek failed.");
            return AVI_ERR_GET_INDEX_ERR;
    ...
    if(mFcc == CDX_CKID_AVI_NEWINDEX)
    {
        ...
        if((CdxStreamTell(aviIn->fp) - aviIn->idx1Start + 1) == (cdx_int64)mLength)
        {
            AviIndexEntryT  mIdx;
            ...
            if(chunk->fcc == CDX_CKID_ODMLHEADER)
            {
                ...
                if(aviIn->hasIndx)
                {
                    if(tmpOdmlHdr->dwTotalFrames > impl->totalFrames)
                    {
                        ...
                        CDX_LOGD("+++ impl->totalFrames: %d", impl->totalFrames);
                    }
                }
        ...
        do
        {
                ...
                CDX_CKTYPE_DIB_COMPRESSED))
            {
                if(1)
                {
                    ...
                    {
                        CDX_LOGV("PROBE_SPECIFIC_DATA_SUCCESS");
                    }
