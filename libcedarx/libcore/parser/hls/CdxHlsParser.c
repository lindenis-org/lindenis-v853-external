...
#define rdLockPlaylist(hlsParser) \
    do { \
        ...
        if (pthread_rwlock_timedrdlock(&hlsParser->rwlockPlaylist, &t)) \
            CDX_LOGD("pthread_rwlock_timedrdlock error: %s", strerror(errno)); \
    } while (0)
...
#define wrLockPlaylist(hlsParser) \
    do { \
        ...
        if (pthread_rwlock_timedwrlock(&hlsParser->rwlockPlaylist, &t)) \
            CDX_LOGD("pthread_rwlock_timedwrlock error %s", strerror(errno)); \
    } while (0)
...
static int SetDataSourceForSegment(CdxHlsParser *hlsParser, PlaylistItem *item)
{
    ...
    if(playlist->u.mediaPlaylistPrivate.hasEncrypte)
    {
        ...
        if(cipherReference != hlsParser->u.media.cipherReference)
        {
            ...
            {
                hlsParser->mSampleKeyItem.key_type = KEY_AES_128;
            }
            else if(!strcasecmp(cipherMethod->mData, "SAMPLE-AES"))
            {
                hlsParser->mSampleKeyItem.key_type = KEY_SAMPLE_AES;
                if(findString(&cipherReference->itemMeta, "cipher-keyformat", &cipherKeyformat))
                {
                    //it is fairplay drm, not support!
                    if(strstr(cipherKeyformat->mData, "com.apple.streamingkeydelivery"))
                    {
                        CDX_LOGE("we support cipher-method:(%s), but it is fairplay drm, not supported.", cipherMethod->mData);
                        ret = -1;
                        goto out;
                    }
                }
            ...
            if (strncasecmp("data:", cipherUri->mData, 5))
            {
            ...
            {
                //maybe it is not the key, please fix it
                cdx_uint32 keyLen = 0;
                char * keySrc = parseDataUriSource(cipherUri->mData, &keyLen);
                if(keySrc == NULL || keyLen < 0)
                {
                    CDX_LOGE("parse key fail!");
                    goto out;
                }
                else
                {
                    memcpy(hlsParser->aesCipherInf.key, keySrc, 16);
                }
        ...
        //SAMPLE-AES decrypt in ts parser --add by ganqiuye
        if(hlsParser->mSampleKeyItem.key_type == KEY_AES_128)
        {
            if(hlsParser->aesUri)
            {
        ...
        if(hlsParser->mSampleKeyItem.key_type == KEY_SAMPLE_AES)
        {
            memcpy(hlsParser->mSampleKeyItem.key, hlsParser->aesCipherInf.key, 16*sizeof(cdx_uint8));
            memcpy(hlsParser->mSampleKeyItem.iv, hlsParser->aesCipherInf.iv, 16*sizeof(cdx_uint8));
            if(hlsParser->child[0] != NULL)
            {
                CdxParserControl(hlsParser->child[0], CDX_PSR_CMD_SET_SAMPLE_ENCRYPT_INFO,
                                   (void *)&hlsParser->mSampleKeyItem);
            }
...
static int DownloadParseM3u8(CdxHlsParser *hlsParser)
{
    ...
    else if(hlsParser->isMasterParser == 0)
    {
        ...
        {
            CDX_LOGD("refreshState(%d)", hlsParser->u.media.refreshState);
...
static void *PeriodicTaskThread(void *arg)
{
    ...
    while (1)
    {
        ...
        {
            CDX_LOGD("mIsComplete=%d, mIsTimeShift=%d",
    ...
    switch(eMessageId)
    {
        ...
        case STREAM_EVT_DOWNLOAD_START:
        {
            ...
            if(!param)
            {
                ...
                if(ret < 0)
                {
                    CDX_LOGD("CDX_PSR_CMD_GET_TS_SEQ_NUM");
                    return -1;
                }
                ...
                if(ret < 0)
                {
                    CDX_LOGD("CDX_PSR_CMD_GET_TS_DURATION");
                    return -1;
                }
                ...
                if(ret < 0)
                {
                    CDX_LOGD("CDX_PSR_CMD_GET_TS_LENGTH");
                    return -1;
                }
                param = (void *)&cur;
            }
...
_NextSegment:
                ...
                if(hlsParser->callback == CallbackProcess && hlsParser->pUserData && (hlsParser->mSampleKeyItem.key_type != KEY_SAMPLE_AES))
                {
    ...
    if(hlsParser->firstPts < 0)
    {
        ...
        CDX_LOGD("hlsParser->firstPts=%lld", hlsParser->firstPts);
    }
...
static cdx_int32 HlsParserSeekTo(CdxParserT *parser, cdx_int64  timeUs, SeekModeType seekModeType)
{
    ...
    else
    {
        ...
        #endif
        /*fix id3v2/aac audio not seek in airplay*/
        if(hlsParser->child[0] &&
          (hlsParser->child[0]->type == CDX_PARSER_ID3V2 ||
           hlsParser->child[0]->type == CDX_PARSER_AAC))
        {
            CdxParserSeekTo(hlsParser->child[0], mDurationUs ,seekModeType);
            CDX_LOGD("found audio,parser type %d seek to %lld...",hlsParser->child[0]->type,mDurationUs);
        }
...
_next:
        ...
        ContorlTask parserContorlTask2;
        parserContorlTask2.cmd = CDX_PSR_CMD_SET_SAMPLE_ENCRYPT_INFO;
        parserContorlTask2.param = (void *)&hlsParser->mSampleKeyItem;
        parserContorlTask2.next = NULL;
        parserContorlTask.next = &parserContorlTask2;
