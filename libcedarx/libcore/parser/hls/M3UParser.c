...
static int decodeBase64(char *out, cdx_int32 *inOutBufSize, const char* s) {
    cdx_int32 n = strlen(s);
    ...
    cdx_int32 outLen = (n / 4) * 3 - padding;
...
/**
 * @brief parseDataUriSource: parse the base64
 ...
 * @return:
 */
char* parseDataUriSource(const char *uri, cdx_uint32* outLen)
{
    char* out = NULL;
    ...
    if(strncasecmp("text/plain;", &uri[5], 11))
    {
        CDX_LOGE("only support text/plain!!");
        return NULL;
    }

    if(!strncasecmp("base64", &uri[16], 6)) {
        ...
        int ret = decodeBase64(out, &len, cleansed);
        free(cleansed);
        if (ret == -1) {
            CDX_LOGE("Malformed base64 encoded content found.");
            return NULL;
        }
        out[len + 1] = '\0';
    } else {
...
static status_t MakeURL(const char *baseURL, const char *url, AString **out)
{
    *out = NULL;
    ...
    #if 0
    else if (!strncasecmp("data:", url, 5))
    {
        //return err_URL;
        cdx_uint32 tmpUrlLen = 0;
        char *tmpUrl = parseDataSource(url, &tmpUrlLen);
        if(tmpUrl == NULL)
            return err_no_memory;
        memSize = n + tmpUrlLen + 2;
        ...
        *(temp + n)='/';
        memcpy(temp + n + 1, tmpUrl, tmpUrlLen + 1);
        free(tmpUrl);
    ...
    while (offset < line->mSize)
    {
        ...
        *equalPos = '\0';
        ...
        else if(!strcmp("keyformat", key))
        {
            ...
            setString(meta, "cipher-keyformat", value);
        }
        value = NULL;/*重要*/
    }
    return OK;
...
static status_t compareAudioGroupID(Playlist *playlist)
{
    status_t err = ERROR_MALFORMED;
    MediaGroup *nMediaGroup = playlist->u.masterPlaylistPrivate.mMediaGroups;
    while(nMediaGroup != NULL)
    {
        if(!strcmp(nMediaGroup->groupID->mData, playlist->nSavedAudioGroupID->mData))
        {
            CDX_LOGD("compareAudioGroupID successed!");
            err = OK;
            break;
        }
        nMediaGroup = nMediaGroup->next;
    }
    if(nMediaGroup == NULL)
    {
        CDX_LOGE("Undefined media group '%s' referenced in stream info.", playlist->nSavedAudioGroupID->mData);
    }
    destroyAString(playlist->nSavedAudioGroupID);
    return err;
}

static status_t saveAudioGroupID(Playlist *playlist, AString *groupID, cdx_uint32 len, char *val)
{
    status_t err = OK;
    if(playlist->nSavedAudioGroupID == NULL)
    {
        playlist->nSavedAudioGroupID = creatAString(val, 1, len - 2);
        if (!playlist->nSavedAudioGroupID)
        {
            CDX_LOGE("Failed to creatAString from '%s'", val);
            err = ERROR_MALFORMED;
        }
        //CDX_LOGD("save audio groupID % s to compare !", playlist->nSavedAudioGroupID->mData);
        playlist->nCompareGroupIDFlag = 1;
    }
    else if (strcmp(playlist->nSavedAudioGroupID->mData, groupID->mData))
    {
         CDX_LOGE("we get two different AudioGroupID '%s' and '%s' ",
                               playlist->nSavedAudioGroupID->mData, groupID->mData);
         err = ERROR_MALFORMED;
    }
...
static status_t parseMedia(AString *line, AMessage *meta, Playlist *playlist)
{
    char *colonPos = strstr(line->mData, ":");
    if(colonPos == NULL)
    {
    ...
    while (offset < line->mSize)
    {
        ...
        *equalPos = '\0';
        ...
        else if (!strcasecmp("group-id", key))
        {
            ...
            groupIDMeta = creatAString(val, 1, len - 2);
            if (!groupIDMeta)
            {
                CDX_LOGE("Failed to creatAString from '%s'", val);
                err = ERROR_MALFORMED;
    ...
    if(mediaGroup == NULL)
    {
        ...
        /* save groupID to itemMeta*/
        if(groupIDMeta)
        {
            if(groupType == TYPE_AUDIO)
                setString(meta, "audio", groupIDMeta);
            else if(groupType == TYPE_SUBS)
                setString(meta, "subtitles", groupIDMeta);
            CDX_LOGD("save groupType %d, groupID %s to itemMeta !",groupType,groupIDMeta->mData);
        }
    ...
    if((playlist->nCompareGroupIDFlag) && (!strcmp("audio",
       playlist->u.masterPlaylistPrivate.mMediaGroups->groupID->mData)))
    {
        err = compareAudioGroupID(playlist);
        if(err != OK)
        {
            CDX_LOGE("The audio GroupID of #EXT-X-STREAM-INF and #EXT-X-MEDIA do not match!");
            goto _err;
        }
    }
    ...
    while (offset < line->mSize)
    {
        ...
        *equalPos = '\0';
                ...
                || !strcmp("subtitles", key))
        {
            ...
            if((mediaGroup == NULL)&&(!strcmp("audio", key)))
            {
                err = saveAudioGroupID(playlist, groupID, len, val);
                if(err != OK)
                {
                     CDX_LOGE("fail to save audio groupID." );
                     goto _err;
                }
             }
    ...
    *P = NULL;
    ...
    while (offset < size)
    {
        ...
        else
        {
            ...
            {
                err = parseMedia(&line, &itemMeta,playList);
            }
