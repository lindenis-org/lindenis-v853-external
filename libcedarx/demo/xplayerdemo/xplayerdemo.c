...
extern SoundCtrl* TinaSoundDeviceSetVolume();
...
#define COMMAND_SET_LOOP        0x10b
#define COMMAND_SET_VOLUME      0x10c   //* set playback volume (use uci)
#define COMMAND_SET_ROTATE      0x10d   //* set real-time rotate degree
...
static const Command commands[] =
{
    ...
    {"set loop",        COMMAND_SET_LOOP,               "setloop ."},
    {"set volume",      COMMAND_SET_VOLUME,               "setvolume ."},
    ...
    {"rotate",    COMMAND_SET_ROTATE, "set real-time rotate degree, 0-0, 1-90, 2-180, 3-270."},
    {NULL, 0, NULL}
};
...
static int parseCommandLine(char* strCommandLine, int* pParam)
{
    ...
    switch(nCommandId)
    {
        ...
        case COMMAND_SET_VOLUME:
            ...
            {
                printf("no audio volume index is specified.\n");
                nCommandId = -1;
            }
            break;
	case COMMAND_SET_ROTATE:
            if(strParam != NULL)
            {
                *pParam = (int)strtol(strParam, (char**)NULL, 10);  //set rotate;
                if(errno == EINVAL || errno == ERANGE)
                {
                    printf("set rotate is not valid.\n");
                    nCommandId = -1;
                }
            }
            else
            {
                printf("set rotate is specified.\n");
                nCommandId = -1;
            }
            break;
...
int main(int argc, char** argv)
{
    ...
    while(!bQuit)
    {
        ...
        {
            //* parse command.
            logd("read command:%s", strCommandLine);
            nCommandParam = 0;
            ...
            switch(nCommandId)
            {
                ...
                {
                    //char* pUrl = "file:///mnt/UDISK/nobody_15s.mp4";
                    char *pUrl = (char*)(uintptr_t)nCommandParam;
                ...
                case COMMAND_SET_LOOP:
                {
                    ...
                    {
                        printf("error:\n");
                        printf("    AwPlayer::setLooping() return fail.\n");
                        break;
                    }
                    printf("set looping.\n");
                    break;
                ...
                case COMMAND_SET_VOLUME:
                {
                    int nAudioVolumeIndex;
                    nAudioVolumeIndex = nCommandParam;
                    printf("switch audio volume to the %dth .\n", nAudioVolumeIndex);
                    TinaSoundDeviceSetVolume(sound, nAudioVolumeIndex);
                    break;
                }
                case COMMAND_SET_ROTATE:
                {
                    int nRotateDegree;
                    nRotateDegree = nCommandParam;
                    logd("COMMAND_SET_ROTATE: set degree is %d", nRotateDegree);
                    //XPayerSetRealTimeRotateDegree(demoPlayer.mAwPlayer, nRotateDegree);
                    break;
                }
